...
if ($_GET['cmd'] === 'add_device') {
  ...
  exec($pushScript . ' "设备注册成功" "设备 ' . $deviceName . ' 已注册" "INFO" 2>&1', $pushOut, $pushCode);
  file_put_contents("$configDir/add_device.log", json_encode(['time' => time(), 'code' => $pushCode, 'output' => $pushOut]));
